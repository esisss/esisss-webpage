...
import { initPointer, updateParallax } from "./parallax/pointer";
...
if (canvas) {
  ...
  // 7. Load Mountains asynchronously then start render loop
  async function init() {
    try {
      const { farMountains, midMountains } = await createMountains();
      scene.add(farMountains);
      scene.add(midMountains);
    } catch (error) {
      console.error("Failed to load mountain assets:", error);
    }

    try {
      const oldCastle = await createOldCastle({
      ...
      console.error("Failed to load old castle asset:", error);
    }

    try {
      boatController = await createBoat();
      scene.add(boatController.boatGroup);

      const passenger = await createBoatPassenger({
        position: { x: 2, y: 1, z: -10 },
        rotation: { x: 0, y: Math.PI * -0.2, z: 0 },
        targetHeight: 1.85,
      });
      boatController.boatGroup.add(passenger);

      atmosphere.setWarmLight(boatController.fireLight);
    } catch (error) {
      console.error("Failed to load boat asset:", error);
    }

    // Start animation loop after mountains are loaded
    animate();
  }
